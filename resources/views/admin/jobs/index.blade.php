...
<section class="content">
    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="box">
                <div class="box-body">
                    <table id="jobs-list" class="table table-striped table-bordered table-hover">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            @foreach($jobs as $job)
                            <tr id="row-{{ $job->id }}" data-id="{{ $job->id }}">
                                <td></td>
                                <td>{{ $job->title }}</td>
                                <td>{{ $job->location }}</td>
...
<script>
    $(function () {
        ...
        $('thead input[name="select_all"]', table.table().container()).on('click', function(e) {
            if (this.checked) {
                $('#jobs-list tbody input[type="checkbox"]:not(:checked)').trigger('click');
            } else {
                $('#jobs-list tbody input[type="checkbox"]:checked').trigger('click');
            }
        ...
        $('#delete_modal').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget),
                ...
                modal = $(this),
                ids = $("#jobs-list tbody input:checked").map(function() {
